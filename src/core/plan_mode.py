...
from ..Runtime.runtime import Jarvis


def _slug(value):
    """
    把任意字符串 → 变成干净、安全、小写、无特殊符号的「短横线链接格式」
    """
...
def _plan_path(topic, path=None):
    if path:
        ...
        value = f".jarvis/plans/{_slug(topic)}-plan.md"
    if (
        not value.startswith(".jarvis/plans/")
        or value.endswith("/")
        or ".." in value.split("/")
    ):
