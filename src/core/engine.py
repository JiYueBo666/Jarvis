...
from src.core.engine_helpers import (
...
from src.core.model_errors import finish_model_error
...
class Engine:
    ...
    def run_turn(self, user_message):
        ...
        # 两个出口同时工作：session event bus 写 JSONL 到磁盘，yield 把事件推给调用方（TUI 用来更新界面）。
        ...
        """
        session event bus：会话生命周期（什么时候开始、什么时候结束、中间有哪些 turn）
        run trace：单次运行的诊断链（prompt 构建耗时、模型调用耗时、工具执行详情）
        """
        agent.session_event_bus.emit(
            "user_message",
        ...
        while tool_steps < agent.max_steps and attempts < max_attempts:
            ...
            attempts += 1
            task_state.record_attempt()  # 改变了task state
            ...
            agent.emit_trace(
                ...
                event="prompt_build",
                payload={
                    "prompt_meta": prompt_metadata,
                    "duration_ms": int((time.monotonic() - prompt_started_at) * 1000),
                },
            )
            """
            checkpoint只创建，在prompt中恢复状态
            """
