...
def build_runtime_event(runtime, task_state, event, payload):
    ...
    payload.setdefault(
        "estimated_input_tokens", int(payload.get("estimated_input_tokens", 0) or 0)
    )
    payload.setdefault(
        "estimated_output_tokens", int(payload.get("estimated_output_tokens", 0) or 0)
    )
    payload.setdefault("artifact_paths", list(payload.get("affected_paths", []) or []))
    payload.setdefault("error_type", _error_type(payload))
    payload.setdefault(
        "parent_span_id", runtime._last_trace_span_id.get(task_state.run_id, "")
    )
